 ...
 *
 *=========================================================================*/
#ifndef itkHoughTransform3DLinesImageFilter_hxx
#define itkHoughTransform3DLinesImageFilter_hxx
...
HoughTransform3DLinesImageFilter< TInputPixelType, TOutputPixelType >
::HoughTransform3DLinesImageFilter()
{
...
::GenerateOutputInformation()
{
  ...
  size[0] = (SizeValueType)m_AngleResolution;
  size[1] = (SizeValueType)m_AngleResolution;
  size[2] = (SizeValueType)( std::sqrt(std::pow(input->GetLargestPossibleRegion().GetSize()[0],2) +
				       std::pow(input->GetLargestPossibleRegion().GetSize()[1],2) +
				       std::pow(input->GetLargestPossibleRegion().GetSize()[2],2)) );
...
::GenerateData()
{
  itkDebugMacro(<< "HoughTransform3DLinesImageFilter called");
  ...
  while ( !image_it.IsAtEnd() )
    ...
    if ( image_it.Get() > m_Threshold )
      {
      for ( double theta = -nPI; theta < nPI; theta += nPI / m_AngleResolution )
        {
	for ( double phi = -nPI; phi < nPI; phi += nPI / m_AngleResolution )
	  {
	  // m_Theta
	  index[0] = (IndexValueType)( ( m_AngleResolution / 2 ) + m_AngleResolution * theta / ( 2 * nPI ) );
	  // m_Phi
	  index[1] = (IndexValueType)( ( m_AngleResolution / 2 ) + m_AngleResolution * phi / ( 2 * nPI ) );
	  // m_R
	  index[2] = (IndexValueType)( image_it.GetIndex()[0] * std::sin(phi) * std::cos(theta) + 
				       image_it.GetIndex()[1] * std::sin(phi) * std::sin(theta) +
				       image_it.GetIndex()[2] * std::cos(theta) );
...
template< typename TInputPixelType, typename TOutputPixelType >
typename HoughTransform3DLinesImageFilter< TInputPixelType, TOutputPixelType >::LinesListType &
HoughTransform3DLinesImageFilter< TInputPixelType, TOutputPixelType >
::GetLines(unsigned int n)
{
  ...
  typedef Image< InternalImagePixelType, 3 > InternalImageType;
  ...
  do
    ...
    for ( it_input.GoToBegin(); !it_input.IsAtEnd(); ++it_input )
      ...
      if ( it_input.Get() == max )
        ...
        double theta   = ( it_input.GetIndex()[0] * 2 * nPI / this->GetAngleResolution() ) - nPI;
	double phi = ( it_input.GetIndex()[1] * 2 * nPI / this->GetAngleResolution() ) - nPI;
        double radius = it_input.GetIndex()[2];
        double Vx = radius * std::sin(phi) * std::cos(theta);
        double Vy = radius * std::sin(phi) * std::sin(theta);
	double Vz = radius * std::cos(theta);
        double norm = std::sqrt(Vx * Vx + Vy * Vy + Vz * Vz);
        double VxNorm = Vx / norm;
        double VyNorm = Vy / norm;
	double VzNorm = Vz / norm;
...
// ???
	...
	LinePointType p;
	p.SetPosition(Vx, Vy, Vz);
	list.push_back(p);
	p.SetPosition(Vx + VxNorm * 5, Vy + VyNorm * 5, Vz + VzNorm * 5);
	list.push_back(p);
	...
	for ( double theta = 0; theta <= 2 * nPI; theta += nPI / 1000 )
	  {
	  for ( double phi = 0; phi <= 2 * nPI; phi += nPI / 1000 )
	    {
	    for ( double length = 0; length < m_DiscRadius; length += 1 )
	      {
	      index[0] = (IndexValueType)( it_input.GetIndex()[0] + length * std::sin(phi) * std::cos(theta) );
	      index[1] = (IndexValueType)( it_input.GetIndex()[1] + length * std::sin(phi) * std::cos(theta) );
	      index[2] = (IndexValueType)( it_input.GetIndex()[2] + length * std::cos(theta) );
